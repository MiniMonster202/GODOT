...
class SpirteFrame extends RefCounted:
	var name = ""
...
func _parseTexturePackerXML(xmlContent):
	...
	if OK == xmlParser.open_buffer(xmlContent.to_utf8_buffer()):
...
func _parseTexturePackerJson(jsonContent):
	...
	test_json_conv.parse(jsonContent):
	if OK == jsonParser.test_json_conv.get_data()
		atlas = {}
		atlas["sprites"] = sprites
		...
		if keys.size() > 0:
			if jsonParser[keys[0]].has("dst") and jsonParser[keys[0]].has("hash"):
				...
				for key in keys:
					...
					sprite["rotation"] = -deg_to_rad(f["rotate"])
					if f["rotate"] == 90:
						sprite["width"] = f["h"]
						sprite["height"] = f["w"]
					sprites.append(sprite)
...
func _parseGDXTexturePacker(jsonContent):
	var strArr =jsonContent.split("\n",true)
	var atlas = null
	var sprites = []
	atlas = {}
	atlas["sprites"] = sprites
	...
	while i < strArr.size()-1:
		...
		var xyarr =  xy[1].split(",")
		sprite["x"] = xyarr[0]
		sprite["y"] = xyarr[1]
		var sizearr =  size[1].split(",")
		sprite["width"] = sizearr[0]
		sprite["height"] = sizearr[1]
		sprite["pivotX"] = 0
		sprite["pivotY"] = 0
		sprite["orignX"] = 0.0
		sprite["orignY"] = 0.0
