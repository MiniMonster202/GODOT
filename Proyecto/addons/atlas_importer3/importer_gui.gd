...
func _ready():
	typeButton.select(0)
	fileDialog.connect("file_selected", Callable(self, "_fileSelected"))
	fileDialog.connect("dir_selected", Callable(self, "_fileSelected"))
	add_child(fileDialog)

	sourceBrowse.connect("pressed", Callable(self, "_on_SourceBrowse_pressed"))
	targetBrowse.connect("pressed", Callable(self, "_selectTargetFile"))
	folderBrowse.connect("pressed", Callable(self, "_selectFolderFile"))
	typeButton.connect("item_selected", Callable(self, "_typeSelected"))
	
	$Input/Button.connect("pressed", Callable(self, "_saveAtlas"))
...
func _on_SourceBrowse_pressed():
	...
	fileDialog.set_mode(FileDialog.FILE_MODE_OPEN_FILE)
	print(typeButton)
...
func _selectFolderFile():
	...
	fileDialog.clear_filters()
	fileDialog.set_access(FileDialog.ACCESS_RESOURCES)
	fileDialog.set_mode(FileDialog.FILE_MODE_OPEN_DIR)
	_showFileDialog()

func _saveAtlas():
	print("Save Atlas")
	if sourceText.text.is_empty():
		printerr("Source is required")
		return
	if folderText.text.is_empty():
		printerr("Save Folder is required")
		return
	if targetText.text.is_empty():
		printerr("Texture2D is required")
		return
	...
	for s in atlas.sprites:
		...
		atex.set_region_enabled(s.region)
		ResourceSaver.save(ap, atex)
	

func _getParentDir(path):
	var fileName = path.substr(0, path.rfind("/"))
	return fileName

func _getFileName(path):
	var fileName = path.substr(path.rfind("/")+1, path.length() - path.rfind("/")-1)
	var dotPos = fileName.rfind(".")
	if dotPos != -1:
