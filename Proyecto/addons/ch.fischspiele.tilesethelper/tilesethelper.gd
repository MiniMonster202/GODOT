...
func _enter_tree():
	dock = preload("res://addons/ch.fischspiele.tilesethelper/tilesethelper_dock.tscn").instantiate()
	#image
	dock.get_node(mainGuiPath+"HBoxImage/VBoxImage/sizeBox/size").connect("text_changed", Callable(self, "tilesize"))
	dock.get_node(mainGuiPath+"HBoxImage/VBoxImage/sizeBox/size").set_text(str(tileSize))
	dock.get_node(mainGuiPath+"HBoxImage/CheckBox").connect("toggled", Callable(self, "setImageCheck"))
	#frames
	...
	#dialog
	dock.get_node(mainGuiPath+"HBoxImage/ImageContainer/btnImage").connect("pressed", Callable(self, "show_dialog"))
	#collision
	dock.get_node(mainGuiPath+"HBoxCollision/collisionPolygon").connect("pressed", Callable(self, "collisionPolygon"))
	dock.get_node(mainGuiPath+"HBoxCollision/CheckBox").connect("toggled", Callable(self, "setCollisionPolygonCheck"))
	#navigation
	dock.get_node(mainGuiPath+"HBoxNavigation/navigation").connect("pressed", Callable(self, "navigation"))
	dock.get_node(mainGuiPath+"HBoxNavigation/CheckBox").connect("toggled", Callable(self, "setNavigationCheck"))
	#occluder
	dock.get_node(mainGuiPath+"HBoxOccluder/occluder").connect("pressed", Callable(self, "occluder"))
	dock.get_node(mainGuiPath+"HBoxOccluder/CheckBox").connect("toggled", Callable(self, "setOccluderCheck"))
	#settings
	dock.get_node(mainGuiPath+"HBoxSettings/CheckGetPolyColli").connect("toggled", Callable(self, "setGetPolygonFromCollisionCheck"))
	#dock.get_node(mainGuiPath+"HBoxSettings/CheckGetPolyColli").set_toggle_mode(getPolygonFromCollision)
	#tiles
	dock.get_node(mainGuiPath+"create_tiles").connect("pressed", Callable(self, "create_tiles"))
...
func setNavigation(_selectedNode):
	if _selectedNode.get_owner() != null:
		if _selectedNode is Sprite2D:
			if _selectedNode.has_node("NavigationRegion2D"):
				_selectedNode.remove_child(_selectedNode.get_node("NavigationRegion2D"))
			var _newNavigationPolygonNode = NavigationRegion2D.new()
			_newNavigationPolygonNode.set_navigation_polygon(getNavPolygon(_selectedNode))
			_newNavigationPolygonNode.set_name("NavigationRegion2D")
...
func getVector2ArrayFromSprite(_selectedNode):
	var _Array = []
	var _Vector2Array = PackedVector2Array(_Array)
...
func getNavPolygon(_selectedNode):
	...
	_navPoly.add_polygon(PackedInt32Array([0, 1, 2, 3]))
...
func show_dialog():
	...
	fileDialog.set_mode(FileDialog.FILE_MODE_OPEN_FILES)
	...
	fileDialog.show()
	if not fileDialog.is_connected("files_selected", Callable(self, "on_files_selected")):
		fileDialog.connect("files_selected", Callable(self, "on_files_selected"))
...
func on_files_selected(_imagePathArray):
	...
	if _imagePathArray.size() == 1:
		...
		if _newTextureWidth > 64 || _newTextureHeight > 64:
			_newTexture.set_size_2d_override(Vector2(64,64))
			...
			for _frame in range(_startFrame,_endFrame):
				...
				if !_root.has_node(_imageName):
					...
					if (int(tileSize) < _newTexture.get_size().x):
						_newSpriteNode.set_region_enabled(true)
						var tmpX = offsetX
...
func getFileName(_path):
	var _fileName = _path.substr(_path.rfind("/")+1, _path.length() - _path.rfind("/")-1)
	var _dotPos = _fileName.rfind(".")
	if _dotPos != -1:
